...
export default function SellerInquiriesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data } = await supabase.from('inquiries').select('*, listings(title, asking_price)').in('listing_id', ids).order('created_at', { ascending: false });
      setInquiries(data || []);
      setLoading(false);
    };
    load();
  ...
  return (
    <div>
      <div style={{ marginBottom: 'var(--space-6)' }}>
        ...
        <p className="page-subtitle">Manage buyer inquiries for your listings</p>
      </div>
      {loading ? (
        <div className="skeleton" style={{ height: 300, borderRadius: 12 }}></div>
      ) : inquiries.length === 0 ? (
        <div className="empty-state"><div className="empty-state-icon">📬</div><h3 className="empty-state-title">No inquiries yet</h3><p className="empty-state-text">Once buyers contact you, their inquiries will appear here.</p></div>
      ) : (
