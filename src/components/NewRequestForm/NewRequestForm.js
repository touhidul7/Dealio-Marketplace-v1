...
import { REQUEST_TYPES, COMPLIANCE_CHECKBOX_TEXT, scanForProhibitedKeywords } from '@/lib/requestsConstants';
import { INDUSTRIES } from '@/lib/constants';
import styles from './NewRequestForm.module.css';

/**
 * Reusable New Request form component.
 * Can be rendered inside any dashboard layout or the public page.
 * @param {{ backHref?: string, backLabel?: string }} props
 */
export default function NewRequestForm({ backHref = '/requests', backLabel = 'Back to Requests' }) {
      ...
      <div className={styles.successWrap}>
        <div className={styles.successCard}>
          ...
          <div className={styles.successActions}>
            <Link href={backHref} className="btn btn-primary">Browse Requests</Link>
        ...
        <div className={styles.stepContent}>
          ...
          <div className={styles.formSection}>
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={formData.timeline} onChange={e => handleFieldChange('timeline', e.target.value)}>
          ...
          <div className={styles.formSection}>
            ...
            {typeConfig.fields.map(field => (
              <div className="form-group" key={field.name}>
                ...
                {field.type === 'text' && (
                  <input className="form-input" placeholder={field.placeholder || ''} value={dynamicFields[field.name] || ''} onChange={e => handleDynamicChange(field.name, e.target.value)} />
                )}
                {field.type === 'number' && (
                  <input type="number" className="form-input" placeholder={field.placeholder || ''} value={dynamicFields[field.name] || ''} onChange={e => handleDynamicChange(field.name, e.target.value)} />
                )}
                {field.type === 'textarea' && (
                  <textarea className="form-textarea" placeholder={field.placeholder || ''} value={dynamicFields[field.name] || ''} onChange={e => handleDynamicChange(field.name, e.target.value)} rows={3} />
                )}
                {field.type === 'select' && (
                  <select className="form-select" value={dynamicFields[field.name] || ''} onChange={e => handleDynamicChange(field.name, e.target.value)}>
          ...
          <div className={styles.stepActions}>
            ...
            <button className="btn btn-primary btn-lg" disabled={!canProceedToStep3()} onClick={() => setStep(3)}>
              Review Request
            </button>
          </div>
        </div>
